...
POLICY_REGEX = %r{(?<min>\d+)-(?<max>\d+) (?<letter>[a-z]): (?<password>.*)};
POLICY2_REGEX = %r{(?<pos1>\d+)-(?<pos2>\d+) (?<letter>[a-z]): (?<password>.*)};

def valid1?(min: , max: , letter:, password:)
  min = min.to_i
  max = max.to_i
  count = password.chars.select{|c| c == letter}.count
  return false if count < min
  return false if count > max
  return true
end

def valid2?(pos1: , pos2: , letter:, password:)
  pos1 = pos1.to_i - 1
  pos2 = pos2.to_i - 1
  chars = password.chars
  [chars[pos1.to_i], chars[pos2.to_i]].select{|c| c == letter}.count == 1
end

def parse(row)
  match = row.match(POLICY_REGEX)
  Hash[ match.names.zip( match.captures ) ].symbolize_keys
end

def parse2(row)
  match = row.match(POLICY2_REGEX)
  Hash[ match.names.zip( match.captures ) ].symbolize_keys
end

def part1
  valid_count = input.select do |row|
    valid1?(parse(row))
  end.count

  puts "part1: valid_count: #{valid_count}"
end
...
def part2
  valid_count = input.select do |row|
    valid = valid2?(parse2(row))
    puts "valid: #{valid} row: #{row}"
    valid
  end.count

  puts "part2: valid_count: #{valid_count}"
end
